...
while True:
    ...
    if codigoRespuestaH == 200:
        print(f'Ok de HTTP post Humedad_DHT11 codigo = {codigoRespuestaH}')
    else:
        print(f'Error HTTP Humedad_DHT11 : {respuestaH.reason.decode('utf-8')}')
        display.fill(0)
        display.text('T.Adaf.IO post', 0, 0, 1)
        display.text(ip, 0, 10, 1)
        display.text('Error HTTP Hum', 0, 30, 1)
        display.text(str(codigoRespuestaH), 0, 40, 1)
        display.text(respuestaH.reason.decode('utf-8'), 0, 50, 1)
        display.show()
        break
    ...
    if codigoRespuestaT == 200:
        print(f'Ok de HTTP post Temperatura codigo = {codigoRespuestaT}')
    else:
        print(f'Error HTTP Temperatura : {respuestaT.reason.decode('utf-8')}')
        display.fill(0)
        display.text('T.Adaf.IO post', 0, 0, 1)
        display.text(ip, 0, 10, 1)
        display.text('Error HTTP Temp', 0, 30, 1)
        display.text(str(codigoRespuestaT), 0, 40, 1)
        display.text(respuestaT.reason.decode('utf-8'), 0, 50, 1)
        display.show()     
        break       
