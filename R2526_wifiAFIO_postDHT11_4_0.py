...
try:
    ...
    errstr = str(e)
    print(errstr)
    display.text('Nok ' + errstr[29:], 0, 10, 1)
    display.show()
    sys.exit(1) # Código de salida opcional
...
while True:
    ...
    try:
        ...
        if codigoRespuestaH != 200:
            ...
            display.text('Error HTTP Temp', 0, 10, 1)
            display.show()
        ...
        sleep(60) # Si hay error http evita bucles de error rápidos
        display.fill_rect(0, 10, 128, 8, 0)
        display.show()
        ...
        if e.errno == 110:
            print('Error de time out en lectura de sensor')
            display.fill_rect(0, 10, 128, 8, 0)
