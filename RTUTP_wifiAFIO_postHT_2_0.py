...
urlAFIO = "https://io.adafruit.com/api/v2/" + secrets['AFIOT_USERNAME'] + "/feeds/" + NOMBREFEEDH + "/data"
...
display.text('Temp. = ' + str(round(temperatura, 2)), 0, 40, 1)
display.show()
...
display.show()
# 3.1 componemeos en 'end point'
urlAFIO = "https://io.adafruit.com/api/v2/" + secrets['AFIOT_USERNAME'] + "/feeds/" + NOMBREFEEDT + "/data"
respuesta = requests.post(urlAFIO, headers = ENCABEZADO, data=ujson.dumps(Temp2d))

# 4 - Mostramos la respuesta: formatamos y la mostramos en el display
# 4.1 - primero el status code
codigoRespuesta = respuesta.status_code
display.text(str(codigoRespuesta), 100, 50, 1)
display.show()
